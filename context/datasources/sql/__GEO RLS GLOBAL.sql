...
(
    ...
    left join "catalog__LevelAccess" a
        on a.id = r.level
...
union all
    ...
    (
							        ...
							        from portal."UserAccessRoles" r
							        left join "catalog__LevelAccess" a_1
							            on a_1.id = r.level
							        where 1 = 1
							          and r.level = 1
    ) u
    left join "catalog__RegionName" o
        on o.id = u._region_id
    left join(
							        select distinct
        ...
        on a._region_name = o."Region Name"
union all
								    select
    ...
    left join catalog__iss_domain i
        on i.id = u.iss_domain_id
    left join(
							        select distinct
							            a_1.iss_domain
							          , a_1."Region Name"
...
set _code = current_setting('dev.geo_rls_global')
where 1=1
	and _report = 'ALL'
	and _page = 'GOE RLS GLOBAL'
