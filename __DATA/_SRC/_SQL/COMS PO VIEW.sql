...
-- destination
		...
		-- when _crd is null or _del is null => null
					,case 
						when coalesce(
								(select el ->> 'value'
								      from jsonb_array_elements(
								...
								or pol.po_app_dt is null then 0 
...
--									and cv."Bill Of Entry Date" is not null / removed 2026-09-05
		...
		,_calc as (
						select 
						    ...
						    ,case 
								when coalesce(_departure_date, _full_etd) is not null
								and coalesce(_arrival_date, _effective_eta) is not null
									then coalesce(_arrival_date, _effective_eta) - coalesce(_departure_date, _full_etd)
								else null end 																									_etd_2_eta
						    ,case 
								when coalesce(_arrival_date, _effective_eta) is not null
									then _del 
											- coalesce(_arrival_date, _effective_eta)
...
--									end																											_status
							...
							,case 
								when coalesce(_arrival_date,_effective_eta) is null 
									or coalesce(_departure_date, _full_etd) is null 
									or _origin_country is null
									 	then 0
								when (coalesce(_arrival_date,_effective_eta) - coalesce(_departure_date, _full_etd)) <= _country_lead_time
									then 1
								when (coalesce(_arrival_date,_effective_eta) - coalesce(_departure_date, _full_etd)) > _country_lead_time
									 then 1 - abs((coalesce(_arrival_date,_effective_eta) 
									 			- coalesce(_departure_date, _full_etd))::numeric / coalesce(nullif(_country_lead_time,0),1))
								else 0
						    ...
						    ,case
						    		when coalesce(_arrival_date,_effective_eta) is null
						    			then null 
						    		else coalesce(_arrival_date,_effective_eta) - _po_need_by_date
						    end																													_rdd_eta
						    ,case 
						-- if revised ETD <> null => REV ETD - CRD actual 
						    		when coalesce(_departure_date, _full_etd) is not null 
						    			then coalesce(_departure_date, _full_etd) - _crd
								else null
						    end																													_crd_2_etd
						    ,case 
		...
		,_main as (
					select 
					    ...
					    ,case 
					    	when z._status in ('Cancelled','Approved', 'Pending','TBA','N/A','','Not Due','Due')
					    		then null
						...
						,case 
							when z._status in ('Cancelled','Approved', 'Pending','TBA','N/A','','Not Due','Due')
				    				then null
					    		when (coalesce(_arrival_date,_effective_eta)) is not null
					    			then (coalesce(_arrival_date,_effective_eta)) - z._po_need_by_date
					    		else null
							end																													_nbd_2_arr
		...
		select 
			...
			,_effective_eta - _po_need_by_date																									_eta_delay
			,case 
				when _po_need_by_date is null or _effective_eta is null then null
				when (_effective_eta - _po_need_by_date) >= 30 then 'Delayed (30+ days)'
				when (_effective_eta - _po_need_by_date) >= 15 then 'Delayed (15-29 days)'
				when (_effective_eta - _po_need_by_date) > 0 then 'Delayed (< 15 days)'
				when (_effective_eta - _po_need_by_date) <= 0 then 'On time / Early'
			...
			,case 
				...
				when (_crd - _po_need_by_date) <= 0 then 'Cargo ready on/before need-by-date (' || abs(_crd - _po_need_by_date) || ' days early)' 
			...
			,coalesce(_crd_2_etd,0) 
			+ coalesce(_etd_2_eta,0) 
			+ coalesce(_eta_2_del,0) 
			+ coalesce(_nbd_2_arr,0) 
			+ coalesce(_nbd_2_del,0)																											_avg_lt
			/*
				Exceptions block
			*/
			,case 
		...
		-- BOOKING PERF exception uses dynamic values fetched from 'SLA_MASTER_COMS' table (see joined table in pre_calc CTE)
			...
			,case 
				when  _departure_date_fallback is not null and _ptd is not null
					then  _departure_date_fallback - _ptd
				when _departure_date_fallback is not null and _etd_iss is not null
					then _departure_date_fallback - _etd_iss
				when _ptd is not null and _etd_iss is null 
					then _etd_iss - _ptd
			...
			,case 
				when  _arrival_date_fallback is not null and _pta is not null
					then  _arrival_date_fallback - _pta
				when _arrival_date_fallback is not null and _eta_iss is not null
					then _arrival_date_fallback - _eta_iss
				when _pta is not null and _eta_iss is null 
					then _eta_iss - _pta
...
--								,null::date 																					_full_eta
			...
			-- LEAD TIME
								...
								,null::text																						_ptd_2_now_bucket 						
								,null::text																						_reason_code
								,null::numeric 																					_nbd_2_crd
								,null::numeric 																					_nbd_2_arr
								,null::numeric 																					_spo_invoice_val_aed
								,null::text 																					_cur_location
								,null::text 																					_port_water
								,null::int	 																					_rdd_2_now
								,null::int 																						_etd_2_crd
								,null::int 																						_rdd_2_pta
								,null::text																						_air_sea_bucket
								,null::int 																						_ptd_2_crd
								,null::int 																						_del_2_ptd
								,null::int 																						_del_2_crd
								,null::numeric 																					_eta_delay
								,null::text 																					_eta_delay_bucket
								,null::text 																					_crd_status
								,null::int 																						_pta_2_rdd
